...
async def startup_event():
    """
    On startup, check if there is an agent configuration file in agent_data/agent_config.json.
    If it exists, seed the agent into Redis.
    """
    try:
        config_path = "agent_data/agent_config.json"
        if os.path.exists(config_path):
            logger.info(f"Found agent configuration at {config_path}. Seeding agent...")
            with open(config_path, "r") as f:
                agent_config_json = json.load(f)
                
                # Basic validation/extraction
                agent_name = agent_config_json.get("agent_name", "default_agent")
                # We use the agent_name as the ID for simplicity in this auto-seed setup, 
                # or generate a UUID if preferred. Let's use a fixed ID for predictability.
                agent_id = "default_agent_id" 
                
                # Prepare data for Redis
                # The structure expected by the server seems to be the raw config + assistant_status
                agent_config_json["assistant_status"] = "seeding"
                
                # Extract prompts if they exist
                agent_prompts = agent_config_json.get("prompts")

                # Handle extraction prompts if any (simplified from create_agent)
                if "tasks" in agent_config_json:
                    for index, task in enumerate(agent_config_json['tasks']):
                        if task['task_type'] == "extraction":
                             # Skip complex prompt generation for auto-seed to avoid startup delays/failures
                             # or implement if strictly necessary. For now, we assume pre-configured or skip.
                             pass

                stored_prompt_file_path = f"{agent_id}/conversation_details.json"
                await asyncio.gather(
                    redis_client.set(agent_id, json.dumps(agent_config_json)),
                    store_file(file_key=stored_prompt_file_path, file_data=agent_prompts, local=True)
                )
                logger.info(f"Agent seeded successfully with ID: {agent_id}")
        else:
            logger.info("No agent_config.json found in agent_data. Skipping auto-seeding.")
